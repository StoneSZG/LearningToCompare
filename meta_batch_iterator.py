...
class MetaBatchIterator:
    ...
    def get_episode(self):
        ...
        # labels = np.array(range(len(selected_classes)))
        # target_label = dict(zip(selected_classes, labels))
        support_set = []
        target_set = []
        for c in selected_classes:
            ...
            # print(c, 'selected_classes:', selected_classes, 'selected_samples:', len(selected_samples))
...
if __name__ == '__main__':
    ...
    for j in range(100):
        ...
        print('x1:', np.shape(x1), 'x2:', np.shape(x2), 'y:', np.shape(y))
        exit(0)
